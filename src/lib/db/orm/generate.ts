...
const extractBalanced = (value: string, openIndex: number): { content: string; endIndex: number } => {
    let depth = 0;
    let inSingleQuote = false;
...
const parseColumnDefinition = (definition: string): ColumnMeta => {
    ...
    const keywordIndex = findKeywordIndex(tail, ["NOT NULL", "NULL", "DEFAULT", "PRIMARY KEY", "UNIQUE", "REFERENCES", "CHECK"]);
    ...
    while (remainder.length > 0) {
        ...
        if (/^DEFAULT\b/i.test(remainder)) {
            const defaultMatch = remainder.match(/^DEFAULT\b\s+([\s\S]+?)(?=\s+(?:NOT NULL|NULL|PRIMARY KEY|UNIQUE|REFERENCES|CHECK)\b|$)/i);
            if (!defaultMatch) {
        ...
        if (/^REFERENCES\b/i.test(remainder)) {
            ...
            const onDeleteMatch = after.match(/^ON DELETE\b\s+([A-Z ]+?)(?=\s+(?:NOT NULL|NULL|DEFAULT|PRIMARY KEY|UNIQUE|REFERENCES|CHECK)\b|$)/i);
            if (onDeleteMatch) {
...
const parseTableConstraint = (definition: string): TableConstraint => {
    ...
    const uniqueMatch = normalized.match(/^(?:CONSTRAINT\s+[a-zA-Z_][\w]*\s+)?UNIQUE\s*\(([^)]+)\)$/i);
    if (uniqueMatch) {
        return {
            type: "unique",
    ...
    const primaryKeyMatch = normalized.match(/^(?:CONSTRAINT\s+[a-zA-Z_][\w]*\s+)?PRIMARY KEY\s*\(([^)]+)\)$/i);
    if (primaryKeyMatch) {
        return {
            type: "primaryKey",
    ...
    const checkMatch = normalized.match(/^(?:CONSTRAINT\s+[a-zA-Z_][\w]*\s+)?CHECK\s*\(([\s\S]+)\)$/i);
    if (checkMatch) {
        return {
            type: "check",
...
const foreignKeysForTable = (table: TableMeta): Extract<TableConstraint, { type: "foreignKey" }>[] => {
...
const zodExpressionForColumn = (table: TableMeta, column: ColumnMeta): string => {
    ...
    } else if (rawType === "INT" || rawType === "INTEGER" || rawType === "SMALLINT" || rawType === "BIGINT") {
    ...
    const checks = [...column.checks, ...table.constraints.filter((constraint) => constraint.type === "check").map((constraint) => constraint.expression)];
    for (const check of checks) {
        const minMatch = check.match(new RegExp(`char_length\\(\\s*${column.name}\\s*\\)\\s*>=\\s*(\\d+)`, "i"));
        if (minMatch) {
            expression += `.min(${Number(minMatch[1])})`;
        }

        const maxMatch = check.match(new RegExp(`char_length\\(\\s*${column.name}\\s*\\)\\s*<=\\s*(\\d+)`, "i"));
        if (maxMatch) {
...
const buildRefinements = (table: TableMeta): string[] => {
    ...
    for (const check of checks) {
        ...
        const mentionedFields = columnNames.filter((col) => expression.includes(col.toLowerCase())).sort();
...
const buildTableSchema = (table: TableMeta): string => {
    ...
    const fields = table.columns.map((column) => `    ${column.name}: ${zodExpressionForColumn(table, column)},`).join("\n");
    const refinements = buildRefinements(table);

    const baseSchemaName = `Base${typeName}Schema`;
    const schemaName = `${typeName}Schema`;
    const omitKeys = ["id", ...table.columns.filter((column) => column.primaryKey).map((column) => column.name), "created_at", "updated_at"];
...
const buildUniqueTypeLine = (table: TableMeta): string => {
    ...
    if (compositeUniqueFields.length > 0) {
        const tuples = compositeUniqueFields.map((fields) => `[${fields.map((field) => `"${field}"`).join(", ")}]`);
...
const buildFieldConstants = (table: TableMeta): string => {
    ...
    const entries = table.columns.map((column) => `    ${column.name}: ${quote(column.name)},`).join("\n");
...
const buildGetTableFields = (tables: TableMeta[]): string => {
    const cases = tables
        .map(
            (table) => `        case ${tableConstantNameFromTable(table.name)}TableName:\n            return ${tableConstantNameFromTable(table.name)};`
        )
        .join("\n");
...
const generateDbTypes = (tables: TableMeta[]): string => {
    ...
    return [header, sections, `\n// -------------- Other functions --------------`, buildGetTableFields(tables), `\n// -------------- End of types --------------`].join("\n");
};
